...
for i in range(256):
    ...
    map[k.encrypt(message)]=i

for i in range(256):
    key=bytes([i,0,0,0,0,0,0,0])
    k=des(key, ECB, iv, pad=None, padmode=PAD_PKCS5)
    if k.decrypt(cipher) in map:
        print('key_11',i)
        print('key_12',map[k.decrypt(cipher)])
...
for i in range(256):
    ...
    c=k.encrypt(message)
    for j in range(256):
        key2=bytes([j,0,0,0,0,0,0,0])
        k2=des(key, ECB, iv, pad=None, padmode=PAD_PKCS5)
        c2=k.encrypt(c)
        if cipher==c2:
            print('key_11',i)
            print('key_12',j)
